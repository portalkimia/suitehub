...
const topics=require('./ikatan-larutan-data.cjs');
const project=process.argv[2];if(!project)throw Error('Pass project path');
const out=path.join(__dirname,'ikatan-larutan-ready');fs.mkdirSync(out,{recursive:true});
const media=path.join(out,'Media_Pembelajaran_Kimia_Drive','Paket_Media_Baru');fs.mkdirSync(media,{recursive:true});
const esc=s=>String(s).replace(/[&<>"']/g,x=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[x]));
const runtime=fs.readFileSync(path.join(__dirname,'ikatan-larutan-runtime.js'),'utf8');
...
for(const t of topics){
 const key='chemportal_ikatan-larutan_'+t.id,config={id:t.id,title:t.title,draftKeys:[key]};
