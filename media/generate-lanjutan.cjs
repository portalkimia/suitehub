...
const topics=require('./lanjutan-data.cjs');
const project=process.argv[2];if(!project)throw Error('Pass project path');
const out=path.join(__dirname,'lanjutan-ready');fs.mkdirSync(out,{recursive:true});
const media=path.join(out,'Media_Pembelajaran_Kimia_Drive','Paket_Media_Baru');fs.mkdirSync(media,{recursive:true});
const esc=s=>String(s).replace(/[&<>"']/g,x=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[x]));
const runtime=fs.readFileSync(path.join(__dirname,'lanjutan-runtime.js'),'utf8');
...
for(const t of topics){
 const key='chemportal_lanjutan_'+t.id,config={id:t.id,title:t.title,draftKeys:[key]};
...
<!-- PK-SUBMISSION-V2-END -->
 ...
 fs.writeFileSync(path.join(media,t.file+'_12_Fase_F.html'),html);
}
